...
- Automatic rollback when the downstream SQL/request fails

## Rollback and Atomicity

Files are uploaded before your SQL query runs, but the platform still provides request-level rollback semantics:

- Success response: uploaded files stay in their configured store(s).
- `4xx` or `5xx` response: uploaded files from that request are deleted automatically.
- Thrown exception during SQL/controller execution: uploaded files from that request are deleted automatically.
- Nested paths generated by `relative_file_path_structure` are rolled back using the full stored relative path, not just the filename.
- Empty parent directories are intentionally left in place.

From the API caller's perspective, file upload plus metadata persistence behaves atomically for the request: either the request succeeds and the files remain available, or the request fails and the new files are removed.

Parent-directory cleanup is intentionally skipped because this engine is designed for high-concurrency workloads. Multiple in-flight requests may target the same folder tree, and GUID path segments can be caller-defined or omitted entirely, so the platform cannot safely infer that a directory chain belongs exclusively to the failing request.
