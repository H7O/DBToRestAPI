...
Execute HTTP requests directly from within your SQL queries — perfect for enriching database data with external APIs, validating inputs against third-party services, or orchestrating microservice calls without leaving SQL.
...
Parameters from the HTTP request (query string, body, headers, route) can be used inside the HTTP call configuration:
...
<validate_email>
  ...
  <verb>POST</verb>
  <query><![CDATA[
    -- Call external email validation service with user-provided email
...
- Sensitive credentials in HTTP configurations should use encrypted settings (see [Settings Encryption](#settings-encryption))
